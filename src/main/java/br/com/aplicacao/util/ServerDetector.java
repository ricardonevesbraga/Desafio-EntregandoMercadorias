 ...
 * @author Ricardo Braga (ajustes na rotina e inclusao do Logger)
 */
public class ServerDetector {

	private static Logger logger = LogUtil.getLogger(br.com.aplicacao.util.ServerDetector.class.getName());
	...
	public synchronized static ServerDetector getInstance() {
		if (_instance == null) {
	...
	public static void setSupportsHotDeploy(boolean supportsHotDeploy) {
		...
		if (supportsHotDeploy) {
			logger.info("Server supports hot deploy");
		} else {
			logger.info("Server does not support hot deploy");
		}
	}
	...
	private void _init() {
		...
		if (System.getProperty("external-properties") == null) {
			if (_serverId != null) {
				logger.info("Detected server " + _serverId);
			} else {
				logger.info("No server detected");
			}
		}
		...
		try {
			for (MBeanServer mBeanServer : MBeanServerFactory.findMBeanServer(null)) {
				...
				if (defaultDomain.equals("jboss")) {
					...
					if (version.startsWith("5")) { return true; }
